const ELEMENTS = {
	...
	upgs: [
		...
		{
			desc: `<b class='yellow'>Unlock Mass Dilation.</b>`,
			unl: "(md)",
			cost: D(1e56),
		},
		{
		...
		{
			desc: `<b class='yellow'>Unlock Stars.</b>`,
			unl: "(star)",
			cost: D(1e225),
		},
		{
...

		{
			...
			desc: `Radiation Self-Boosts are 505 better.`,
			cost: D('e9e6')
		}, {
			desc: `Stronger always give +^350 without softcaps.`,
			cost: D('e1e7')
		}, {
			desc: `Unlock the final 3 Atomic upgrades.`,
			unl: "(upg)",
			cost: D('e1.111e7')
		}, {
			desc: `Supernovae scale Radiation Boosters slower.`,
			cost: D('e1.15e7'),
			effect() {
				let x = player.supernova.times.div(1e3)
				return x
			},
			effDesc(x) { return "-^"+format(x) },
		}, {
			desc: `Raise Radiation effects.`,
			cost: D('e1.5e7')
		}, {
			desc: `Tickspeed always scale at ^1.25 rate. Boosts to non-bonuses are disabled.`,
			cost: D('e2e7')
		}, {
			desc: `Mass Upgrades always scale at ^1.25 rate.`,
			cost: D('e2e7')
		}, {
			desc: `Placeholder.`,
			cost: EINF
...
function setupElementsHTML() {
	...
	for (let i = 0; i < ELEMENTS.map.length; i++) {
		...
		if (m=='v') table += '</div><div class="table_center">'
			...
			:`<button class="elements" id="elementID_${num}" onclick="ELEMENTS.buyUpg(${num})" onmouseover="tmp.elements.choosed = ${num}" onmouseleave="tmp.elements.choosed = 0">
				<div style="font-size: 9px;">${num}</div>
				${ELEMENTS.names[num]}
				${ELEMENTS.upgs[num].unl ? '<b style="font-size:9px">'+ELEMENTS.upgs[num].unl+'</b>' : ''}
			</button>`
