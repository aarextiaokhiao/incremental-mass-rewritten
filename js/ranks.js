const RANKS = {
	...
	mustReset(type) {
		if (tmp.chal14comp || tmp.inf_unl) return false
...
const BEYOND_RANKS = {
	...
	getScalings() {
		return {
			tier: tmp.beyond_ranks.tier_power,
			start: tmp.beyond_ranks.scale_start,
			power: tmp.beyond_ranks.scale_pow
		}
	},
    getTier(r=player.ranks.beyond) {
		let s = this.getScalings()
        return r.gt(0)?r.log10().max(0).pow(.8).mul(s.tier).add(1).scale(s.start, s.power, 0, true).floor():E(1)
    },
    getRankFromTier(i,r=player.ranks.beyond) {
		let s = this.getScalings()
        let hp = E(10).pow(E(i).scale(s.start, s.power, 0).sub(1).div(s.tier).pow(1.25)).ceil()
        return r.div(hp).floor()
    },
    getRequirementFromTier(tier, amt) {
		let s = this.getScalings()
        return amt.add(1).mul(E(10).pow(
			E(tier).scale(s.start, s.power, 0).sub(1).div(s.tier).pow(1.25).sub(
			E(tier).scale(s.start, s.power, 0).sub(2).div(s.tier).pow(1.25))
		)).ceil()
    },
    ...
    noReset: _ => hasBeyondRank(2,2)||hasInfUpgrade(10),
    reset(auto=false) {
    ...
    rewardEff: {
        ...
        4: {
            ...
            2: [
                ()=>{
                    let x = tmp.beyond_ranks.max_tier.sub(3).pow(hasBeyondRank(14,1) ? 1 : .2).mul(.2).add(1)
...
function updateRanksHTML() {
    if (tmp.stab[0] == 0) {
        ...
        if (unl) {
            ...
            for (let x = 0; x < 4; x++) {
                let rn = RANKS.names[x]
                h += getScalingName(rn) + RANKS.fullNames[x] + ' <h4 class="ranks_amt">[' + format(player.ranks[rn],0) + ']</h4><br>'
            }
            ...
            let t = tmp.beyond_ranks.max_tier.toNumber()
            h = ''
            let h2 = ''

            for (let x = Math.min(3,t)-1; x >= -1; x--) {
                if (x >= 0) h += getRankTierName(t+5-x) + " <h4 class='ranks_amt'>[" + (x == 0 ? tmp.beyond_ranks.latestRank.format(0) : BEYOND_RANKS.getRankFromTier(t-x).format(0)) + "]</h4>" + (x>0?'<br>':"")
            ...
            for (tt in BEYOND_RANKS.rewards) {
                ...
                for (tr in BEYOND_RANKS.rewards[tt]) {
                    tt = Number(tt)
                    if (t<tt || (tmp.beyond_ranks.latestRank.lt(tr) && t == tt)) {
