const ELEMENTS = {
    ...
    upgs: [
        ...
        {
            desc: `<span class="final_elem">[Final Atomic Element]</span> Normal mass boosts all-star resources at a reduced rate.`,
            cost: E('e5200'),
            effect() {
        ...
        {
            desc: `<span class="final_elem">[Final Supernova Element]</span> Mass softcap^3 is 17.5% weaker.`,
            cost: E('e4e11'),
        },
        {
        ...
        {
            desc: `<span class="final_elem">[Final Quantum Element]</span> Unlock <span class="final_elem">Darkness</span>, you'll able to go Dark.`,
            cost: E("e1.7e17"),
        },
        {
            dark: true,
            ...
            desc: `<span class="final_elem">[Final Darkness Element]</span> Unlock the 16th Challenge.`,
            cost: E('e7e134'),
        },{
            ...
            inf: true,
            desc: `<span class="final_elem">[Final Infinity Element]</span> Unlock 20th Challenge.`,
    ...
    getUnlLength() {
		...
		} else if (player.chal.comps[7].gte(16)) {
			u = 4
    ...
    for (let k = 1; k <= MAX_ELEM_TIERS; k++) {
        ...
        for (let i = 0; i < ELEMENTS.map[k-1].length; i++) {
            ...
            if (m=='v') table += `</div><div class="table_center">`
                ...
                :`<button class="elements" id="elementID_${num}" onclick="buyElement(${num})" onmouseover="tmp.elements.choosed = ${num}" onmouseleave="tmp.elements.choosed = 0">
        ...
        if (unl) {
            ...
            for (let x = tElem.ts+1; x <= len; x++) {
                ...
                if (upg) {
                    ...
                    upg.setVisible(unl2)
                    if (unl2) {
                        let eu = [ELEMENTS, MUONIC_ELEM][elayer].upgs[x]
						...
						upg.setClasses(c16 && isElemCorrupted(x,elayer) ? {
							...
							sn: elayer == 0 && x > 54 && x < 77,
							qu: elayer == 0 && x > 77 && x <= 86,
