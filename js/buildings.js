const BUILDINGS_DATA = {
	...
	mass_2: {
		...
		effect(x, bonus) {
			...
			let ret = step.mul(hasAscension(0,1)?x.add(1).mul(bonus.add(1)):x.add(bonus)).add(1)
	...
	mass_3: {
		...
		get_effect: x => "^"+format(x.effect)+" to Booster Power"+(x.effect.gte(x.ss)?` <span class='soft1'>(softcapped${x.effect.gte(1.8e5)?x.effect.gte(5e15)&&!player.ranks.pent.gte(15)?"^3":"^2":""})</span>`:""),
	},
	mass_4: {
		name: "Overpower",
		icon: "mass_upg4",
	...
	tickspeed: {
		...
		+(x.power.gte(x.ss)&&!hasUpgrade('rp',16)?" <span class='soft1'>(softcapped)</span>":""),
...
const BUILDINGS = {
	...
	update(i) {
		...
		tmp.el["building_auto_"+i].setVisible(b.autoUnlocked)
