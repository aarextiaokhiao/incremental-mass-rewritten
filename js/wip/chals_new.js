const CHALS_NEW = {
	...
	enter(layer, x) {
		...
		if (tmp.chal_new.lastLayer == layer && !toConfirm('chal')) return
	...
	exit(layer) {
		if (tmp.chal_new.lastLayer == layer && player.confirms.chal && !confirm("Exit this challenge? You won't gain additional completions until you return!")) return
		for (const c of player.chal.progress[layer]) gainChalComp(c)
	...
	clear(layer) {
		...
		delete player.chal.progress[layer]
		delete tmp.chal_new.choosed[layer]
	},

	//Choosing
	choose(x) {
		const layer = Math.ceil(x / 4) - 1
		if (tmp.chal_new.choosed[layer] == x) CHALS_NEW.enter(layer, x)
		else tmp.chal_new.choosed[layer] = x
	},
	...
	//Others
	max(x) {
		return x > 8 ? CHALS_NEW.chals[x].max : CHALS_NEW.chals[x].max()
	},
	eff(x, def = D(1)) {
		return tmp.chal_new.eff[x] || CHALS_NEW.chals[x].effect(player.chal.comps[x])
	},
...
function chalTick() {
	...
	for (const i of tmp.chal_new.in) {
...
function updateChalTempNew() {
	...
	tmp.chal_new.lastLayer = lastLayer
	...
	tmp.chal_new.in = [...inChal]
	tmp.chal_new.inLastLayer = inLastLayer

	for (let c = CHAL_NUM; c > 0; c--) {
		let force = CHALS_NEW.chals[c].force
		if (force && force() && !inChal.includes(c)) inChal.push(c)
	}
	tmp.chal_new.inForce = inChal

	//Eff
	tmp.chal_new.eff = {}
	for (let c = CHAL_NUM; c > 0; c--) {
		tmp.chal_new.eff[c] = CHALS_NEW.chals[c].effect(player.chal.comps[c])
	}
}
...
function setupChalHTMLNew() {
	...
	for (let layer = CHALS_NEW.layers.length - 1; layer >= 0; layer--) {
		...
		html += `
			<button id="chals_${layer}_start" class="btn" onclick="CHALS_NEW.enter(${layer}, tmp.chal_new.choosed[${layer}])">Start</button>
...
function updateChalHTMLNew() {
	for (const [id, layer] of Object.entries(CHALS_NEW.layers)) {
		...
		const choosed = tmp.chal_new.choosed[id]
		for (let c = id * 4 + 1; c <= id * 4 + 4; c++) {
			...
			elm[`chal_${c}_comp`].setHTML(
				...
				tmp.chal_new.in.includes(c) ? "+" + format(player.chal.bulkComps[c].sub(player.chal.lastComps[c]), 0) :
				format(player.chal.comps[c] || 0, 0) + " / " + format(CHALS_NEW.max(c), 0))
			elm[`chal_${c}_btn`].setClasses({img_chal: true, ch_choosed: choosed == c, ch_in: inChal && tmp.chal_new.in.includes(c), ch_force: inChal && !tmp.chal_new.in.includes(c), ch_comp: comp && !inChal })
		}
		...
		if (choosed) {
			elm[`chals_${id}_title`].setTxt(`[${choosed}] ${CHALS_NEW.getScaleName(choosed) + CHALS_NEW.chals[choosed].title}`)
			elm[`chals_${id}_desc`].setTxt(CHALS_NEW.chals[choosed].desc)
			elm[`chals_${id}_goal`].setTxt(`${formatMass(CHALS_NEW.goal(choosed))} ${layer.resName}`)
			elm[`chals_${id}_reward`].setHTML(CHALS_NEW.chals[choosed].reward)
			elm[`chals_${id}_eff`].setHTML(CHALS_NEW.chals[choosed].effDesc(tmp.chal_new.eff[choosed]))
		}
	}
...
function updateChalHeaderNew() {
	let layer = tmp.chal_new.inLastLayer
